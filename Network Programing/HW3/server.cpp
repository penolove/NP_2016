...
int recvMessage(int sockfd, string &result) {
	...
	if(pre_buffer == "") {
		while(1) {
			...
			pre_buffer += recv_buff;
			if(recv_buff[len-1] == '\n')			
				break;
		}
	}
	
	for(int i = 0; i < pre_buffer.length(); i++) {
		if(pre_buffer[i] == '\n') {
			result = pre_buffer.substr(0, i);
			pre_buffer = pre_buffer.substr(i+1, pre_buffer.length()-i-1);
			break;
		}
	}
...
int ras_processCommand(deque<command_t> cmd_queue, int id, string cmd_line) {
	...
	while(!cmd_queue.empty()) {
		...
		if(action.pipe_to_user > 0) {
			...
			sprintf(tmp, "*** %s (#%d) just piped '%s' to %s (#%d) ***\n", clients_info[id].name, id, cmd_line.c_str(), clients_info[action.pipe_to_user].name, action.pipe_to_user);
			ras_talk(0, tmp, id);
		}
	}
