...
/* Tools */
int sendMessageReal(client_t &info) {
	int n;
	n = write(info.write_sockfd, info.write_buffer.c_str(), strlen(info.write_buffer.c_str()));
	if(info.write_buffer.length() > n){
		info.write_buffer = info.write_buffer.substr(n);
		return 0;
	}
	return 1;
}
int sendMessage(client_t &info, int sockfd, string message) {
	info.write_buffer = message;
	info.write_sockfd = sockfd;
}
int sendMessage(client_t &info, int sockfd, char* message) {
	string tmp = message;
	info.write_buffer = tmp;
	info.write_sockfd = sockfd;
}
...
int rbs_sendcmd(client_t &info) {
	...
	if(sendMessage(info, info.sockfd, line) <= 0)
		return 0;
...
int main(int argc, char *argv[]) {
	...
	for(int i = 0; i < client_infos.size(); i++) {
		...
		client_infos[i].status = F_CONNECTING;
		FD_SET(client_infos[i].sockfd, &arfds);
		FD_SET(client_infos[i].sockfd, &awfds);
	...
	while(1) {
		...
		for(int id = 0; id < client_infos.size(); id++) {
			/*cout << client_infos[id].id << ": ";
			if(FD_ISSET(client_infos[id].sockfd, &rfds))
				cout << "Reading ";
			if(FD_ISSET(client_infos[id].sockfd, &wfds))
				cout << "Writing ";
			cout << "<br>";*/
			if(client_infos[id].status == F_CONNECTING && (FD_ISSET(client_infos[id].sockfd, &rfds) || FD_ISSET(client_infos[id].sockfd, &wfds))) {
				int error;
				socklen_t n = sizeof(int);
				if (getsockopt(client_infos[id].sockfd, SOL_SOCKET, SO_ERROR, &error, &n) < 0 || error != 0) {
					close(client_infos[id].sockfd);
					fclose(client_infos[id].fp);
					FD_CLR(client_infos[id].sockfd, &arfds);
					FD_CLR(client_infos[id].sockfd, &awfds);
					client_infos.erase(client_infos.begin()+id);
				} else {
					client_infos[id].status = F_READING;
					FD_CLR(client_infos[id].sockfd, &awfds);
				}
			} else if(client_infos[id].status == F_READING && FD_ISSET(client_infos[id].sockfd, &rfds)) {
				int re_val;
				if((re_val = rbs(client_infos[id])) <= 0) {
					...
					client_infos[id].status = F_WRITING;
					FD_SET(client_infos[id].sockfd, &awfds);
				}
			} else if(client_infos[id].status == F_WRITING && FD_ISSET(client_infos[id].sockfd, &wfds)) {
				if(sendMessageReal(client_infos[id])) {
					client_infos[id].status = F_READING;
					FD_CLR(client_infos[id].sockfd, &awfds);
				}
			}
